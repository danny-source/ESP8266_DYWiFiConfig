...
//Use last SPIFFS 4kuint8_ts to store Config
EEPROMClass DYEEPROM((((uint32_t)&_SPIFFS_end - 0x40200000) / SPI_FLASH_SEC_SIZE) - 1);
// EEPROMClass DYEEPROM((((uint32_t)&_EEPROM_start - 0x40200000) / SPI_FLASH_SEC_SIZE));
...
void DYStoreConfig::begin(int allocsize, int storeaddress, DYWIFICONFIG_STRUCT *wifconfig_struct)
{
	...
	DYWIFICONFIG_DEBUG_PRINTLN((((uint32_t)&_SPIFFS_end - 0x40200000) / SPI_FLASH_SEC_SIZE));
...
template <class T>
int DYStoreConfig::read(int address, T &data)
{
...
template <class T>
int DYStoreConfig::write(int address, const T &data)
{
